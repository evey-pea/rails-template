...
for profile in profiles
    p user = User.create(email: "#{test_email_base[0]}-#{email_index}#{test_email_base[1]}", password: "test1234")
    p user_id = {user_id: user["id"]}
    p Profile.create(profile.merge(user_id))
    email_index += 1
end

# Add hug preferences to profiles 